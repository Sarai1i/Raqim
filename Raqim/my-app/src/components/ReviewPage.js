...
const ReviewPage = () => {
  ...
  const updateOriginalHighlight = (wordData, shouldCenter = false) => {
    ...
    // نحسب موضع المربع كنسبة مئوية من أبعاد الصورة الأصلية.
    // هذه الطريقة تتكيّف تلقائيًا مع أي حجم عرض ومع الـ scroll دون حسابات بكسل هشّة.
    const sourceWidth = getBoxMetric(box, "original_width", 0);
    const sourceHeight = getBoxMetric(box, "original_height", 0);
    if (!sourceWidth || !sourceHeight) {
      setHighlightedBox(null);
      return;
    }

    const x = getBoxMetric(box, "x", 0);
    const y = getBoxMetric(box, "y", 0);
    const w = getBoxDimension(box, "w", "width", 1);
    const h = getBoxDimension(box, "h", "height", 1);
    ...
    // تمرير الصورة حتى تظهر الكلمة في منتصف صندوق المعاينة.
    if (shouldCenter && originalPreviewRef.current && imageRef.current) {
      const preview = originalPreviewRef.current;
      const image = imageRef.current;
      const wordTopPx = (y / sourceHeight) * image.clientHeight;
      const targetTop = Math.max(0, wordTopPx - (preview.clientHeight / 2));
