...
from ocr_engine import DeepSeekOCRError, DotsOCRError, ocr_with_highlighting, current_engine_label
...
def process_ocr(file_entry):
    ...
    # تشغيل محرك OCR المُختار (DeepSeek-OCR-2 أو DotOCR) وتحديث `ocr_results`.
    ...
    try:
    ...
    except (DeepSeekOCRError, DotsOCRError) as ocr_error:
        processing_complete = False
        ...
        ocr_engine_status = {
            "engine": "ocr_error",
            "label": f"تعذر تشغيل {engine_label}",
            "provider": "local",
            "error": str(ocr_error),
        }
        print(f"❌ تعذر تشغيل {engine_label}: {ocr_error}")
        return
...
def _build_corrected_docx(path, pages=None):
    """بناء ملف Word (.docx) من نتائج OCR (يفوّض إلى docx_builder المشترك).

    الإعادة بناء (عناوين/فقرات/جداول حقيقية مع RTL) موحّدة بين محرّكي OCR
    (DeepSeek-OCR-2 و DotOCR) لأن كليهما يصدّر نفس بنية الكلمات الموسومة.
    """
    source_pages = pages if pages is not None else ocr_results
    build_corrected_docx(path, source_pages)
