...
def _normalize_for_ocr(image: Image.Image) -> Image.Image:
    """تحسين الصورة قبل OCR: تدرّج رمادي + زيادة تباين + حدّة لتقليل تشويه الحروف."""
    from PIL import ImageFilter

    gray = ImageOps.grayscale(image)
    # قصّ أطراف الهيستوغرام لزيادة التباين بين الحبر والخلفية
    contrasted = ImageOps.autocontrast(gray, cutoff=1)
    # حدّة خفيفة تساعد على فصل الحروف العربية المتصلة
    sharpened = contrasted.filter(ImageFilter.UnsharpMask(radius=1.2, percent=120, threshold=2))
    return sharpened
